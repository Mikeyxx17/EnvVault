...
envvault --vault <PATH> list [--verbose]
envvault --vault <PATH> exists <NAME>
envvault --vault <PATH> rename <CURRENT> <NEW>
envvault --vault <PATH> change-password
envvault --vault <PATH> remove <NAME>
envvault --vault <PATH> import [--dry-run] <SOURCE>
envvault --vault <PATH> example [--output <PATH>] [--profile <PATH>]
...
envvault --vault <PATH> policy grant-inspect --caller-id <CALLER_ID> --profile <PATH>
envvault --vault <PATH> policy list
envvault --vault <PATH> policy revoke-use --caller-id <CALLER_ID> [--profile <PATH>] [--secret <NAME>]
envvault --vault <PATH> audit list [--caller-id <ID>] [--secret <NAME>] [--operation <OP>]
...
envvault --vault <PATH> run [--dry-run] --profile <PATH> --credential-file <PATH> -- <PROGRAM> [ARG]...
envvault completions <bash|zsh|powershell>
```

`--vault`、`--as` 和 `--format` 是全局选项，也可以放在子命令之后。`--as NAME` 选用 `envvault.json` 里 `targets.NAME` 的 profile / credential / caller_id，不覆盖顶层默认。`--format json` 只改变列表和预览类输出，仍禁止 Secret Value。CLI 不接受 `--password`、`--value`、密码/Secret 环境变量或管道输入。
...
- `list` 仅输出获得逐 Secret `list` Allow 的名称；`--verbose` 追加 SecretId 与 `use` Allow 的 caller 标签/ID，不含 Value。
- `rename` 需要 `write`，保留 SecretId 与 Value，只改名称。
- `change-password` 仅限当前 Owner；用新密码重加密 Vault。若存在 machine-unlock binding，用新 Master Key 轮换包装。
- `identity list` 在过期时间后增加 `ok` / `expiring`（14 天内）/ `expired` / `unbounded`。
- `audit list` 可按 `--caller-id`、`--secret`（需 `list` Allow 才能解析名称）和 `--operation` 过滤。
- `run --dry-run` 只输出环境键与 inject/deny/missing，不解密、不启动进程。Deny 与缺失 Secret 的错误不再混用同一句话。
- `import` 严格解析并把每个 key/value 作为独立 Secret 全有或全无地提交；不修改或删除源文件。`--dry-run` 只输出 value-free 的 create/replace/conflict 预览，不提交 Vault。
- 未指定 `--vault` 的 `init` 在写入 `envvault.json` 后确保项目 `.gitignore` 含 `.envvault/` 与 `*.credential.json`；已有文件只追加缺失行。
...
- `--as NAME` 读写 `envvault.json` 的 `targets.NAME`（value-free 路径与 caller_id）。`identity register --as` / `profile create --as` 不覆盖顶层默认 caller/profile。`run` / `session` / `policy grant-*` / `revoke-use` 用该 target 解析缺省路径。未知或未写全的 target 失败关闭。
- `profile create` 只生成环境名到 SecretId 的 value-free 请求集合；`policy grant-use` 才执行显式精确 `use` 授权。
- `policy grant-inspect` 为同一 Profile 写入精确 `list` 与 `exists` Allow，不授予 `use` 或 `read_plaintext`。
- `policy list` 需要 `manage_policy`；输出 generation 与逐条规则，不含 Value。Secret 名称仅在调用者对该 Secret 具有 `list` 时显示。
- `policy revoke-use` 只删除指定 Caller 对指定 SecretId 的 `use` Allow；有 `--secret` 时按名称解析并忽略 Profile，否则使用 Profile 中的 SecretId。身份、inspect 授权和其它 Secret 的 `use` 不受影响。
